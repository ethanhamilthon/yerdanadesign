    ...
    return (
        <div className="relative" ref={dropdownRef}>
            <div
                ...
                className="py-2 px-3 sm:px-4 cursor-pointer gap-2 flex items-center bg-neutral-200 hover:bg-neutral-300 transition-colors border rounded-full"
            >
                <span className="font-bold text-sm sm:text-base text-neutral-700">/{currentLang.label}</span>
                <img src="/language.svg" alt="lang" className={`transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`} />
            </div>

            {isOpen && (
                <div className="absolute top-full right-0 mt-2 py-2 bg-white border border-neutral-200 rounded-2xl shadow-lg z-50 min-w-[100px] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                    {languages.map((lang) => (
                        <button
                            key={lang.code}
                            onClick={() => changeLanguage(lang.code)}
                            className={`px-4 py-2 text-left cursor-pointer text-sm font-medium hover:bg-neutral-100 transition-colors ${i18n.language === lang.code ? 'text-[#ff003c]' : 'text-neutral-700'}`}
                        >
                            {lang.label}
                        </button>
                    ))}
